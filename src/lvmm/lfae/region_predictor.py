...
class RegionPredictor(nn.Module):
    ...
    def region2affine(self, region):
        ...
        grid = make_coordinate_grid(shape[2:],
                                    region.type()).unsqueeze_(0).unsqueeze_(0)
        ...
        if self.pca_based:
            ...
            covar = torch.matmul(mean_sub.unsqueeze(-1),
                                 mean_sub.unsqueeze(-2))
    ...
    def forward(self, x):
        ...
        if self.jacobian is not None:
            ...
            jacobian_map = jacobian_map.reshape(final_shape[0], 1, 4,
            ...
            jacobian = jacobian.view(jacobian.shape[0], jacobian.shape[1], 2,
                                     2)
            region_params['affine'] = jacobian
            region_params['covar'] = torch.matmul(jacobian,
                                                  jacobian.permute(0, 1, 3, 2))
        elif self.pca_based:
