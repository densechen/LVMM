...
class Generator(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_down_blocks):
            ...
            up_blocks.append(
                UpBlock2d(in_features, out_features, kernel_size=3, padding=1))
        ...
        for i in range(num_bottleneck_blocks):
            self.bottleneck.add_module(
                'r' + str(i),
                ResBlock2d(in_features, kernel_size=3, padding=1))
    ...
    def apply_optical(self,
                      input_previous=None,
                      input_skip=None,
                      motion_params=None):
        """
        Args:
            input_previous is deformed feature from previous layer.
            input_skip is un-deformed feature from encoder layer.
            
            If occlusion map is None, input_previous will be added with deformed input_skip
            If occlusion map is not None, input_previous will be added with deformed input_skip with a weight.
        """
        if motion_params is not None:
            ...
            if occlusion_map is not None:
                if input_skip.shape[2] != occlusion_map.shape[
                        2] or input_skip.shape[3] != occlusion_map.shape[3]:
                    occlusion_map = F.interpolate(occlusion_map,
                                                  size=input_skip.shape[2:],
                                                  mode='bilinear')
                if input_previous is not None:
                    input_skip = input_skip * occlusion_map + input_previous * (
                        1 - occlusion_map)
                else:
                    input_skip = input_skip * occlusion_map
            else:
                if input_previous is not None:
                    input_skip = input_skip + input_previous
                else:
    ...
    def forward(self,
        ...
        for i in range(len(self.up_blocks)):
            if self.skips:
                out = self.apply_optical(input_skip=skips[-(i + 1)],
                                         input_previous=out,
                                         motion_params=motion_params)
            out = self.up_blocks[i](out)
        if self.skips:
            out = self.apply_optical(input_skip=skips[0],
                                     input_previous=out,
                                     motion_params=motion_params)
        out = self.final(out)
        ...
        if self.skips:
            out = self.apply_optical(input_skip=source_image,
                                     input_previous=out,
                                     motion_params=motion_params)
    ...
    def forward_with_flow(self, source_image, optical_flow, occlusion_map, cinns_hook=None):
        out = self.first(source_image)
        skips = [out]
        for i in range(len(self.down_blocks)):
            out = self.down_blocks[i](out)
        ...
        output_dict["deformed_feat"] = out
        
        if cinns_hook:
            # call cinns to improve the quality of out feature.
            out = cinns_hook(out)
