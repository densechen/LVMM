...
from lvmm.ldm import FrameDiffusion
...
def train(config, batch_size=2):
    ...
    optimizer = torch.optim.AdamW(
        model.diffusion.parameters(), lr=train_params.lr)
    ...
    while actual_step < final_step:
        ...
        for i_iter, x in enumerate(dataloader):
            ...
            model.module.set_input(
                ref_img=ref_img, real_vid=real_vid, debug=False)
            model.module.prepare_training_data()
            model.module.clear_cache()

            for _ in range(real_vid.shape[2] // config.frame_diffusion_params.mini_batch):
                loss = model.module()
                loss.backward()
            model.module.clear_feat()
            clip_grad_norm_(model.module.diffusion.parameters(), max_norm=1.)
            ...
            if is_main_process:
                writer.add_scalar("loss", total_losses.val,
...
if __name__ == "__main__":
    ...
    config.frame_diffusion_params.mini_batch = args.batch_size
    args.batch_size = 4
