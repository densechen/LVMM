...
def main(config, batch_size=2):
    ...
    if not os.path.exists(config.flow_ae_checkpoints):
        raise FileNotFoundError(f"FlowAE weights missed.")
    else:
    ...
    latent_size = dataset.resolution // 4
    ...
    for i_iter, x in enumerate(dataloader):
        ...
        with torch.no_grad():
            ...
            model.sample_one_video(latent_size=latent_size, cond_scale=1.0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", help="path to config",
                        default="configs/test_lfdm.yaml")
